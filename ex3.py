...
if __name__ == '__main__':
	...
	scale_multiplier = 2
	force_multiplier = 1
	time_multiplier = 0.05*10**-10	#affects on time only. DOESN'T WORK
	white = (255,255,255)
	black = (0,0,0)
	gray = (125,125,125)
	blue = (0,0,255)
	...
	Nparticles_in_row = 3	#for p=1 bar ~115 particles needed for 800x600 px area (1 px = 1 A if scale_multiplier=1)
	Nparticles_in_column = 10
	init_speed = 13.67*10**10*time_multiplier*scale_multiplier
	mass = 40
	for i in range(Nparticles_in_row):
		for j in range(Nparticles_in_column):
			init_speed_bool = random.randint(0, 1)
			...
			init_speed*init_speed_bool*sign_bool, init_speed*(not init_speed_bool)*sign_bool,
	...
	while running:
		...
		Cv = 5/2*8.31
		density = Nparticles_in_column*Nparticles_in_row*mass/(display_width*display_height)*1.66054*10**-3*scale_multiplier**2	#amu/A^2 --> *1.66054e-27/10**-30 kg/m^3
		free_path = calc_free_path()*10**-10	#[m]
		av_speed = calc_av_speed()/time_multiplier*10**-10	#[m/s]
		av_free_path_time = free_path/av_speed #[s]
		Diffusion = 2/3*free_path*av_speed 	#[m^2/s]
		thermal_conductivity = density*Cv*Diffusion
		viscosity = density*Diffusion	#should be around 22.7e-6 Pa*s И НЕ ВАЖНОБ ЧТО ПО РАЗМЕРНОСТИ НЕ СХОДИТСЯ. ПОХУЙ
